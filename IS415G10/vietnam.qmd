...
```{r}
geospatial_provinces <- unique(vietnam_provinces$province_name)

farms_data <- farms_data %>%
    filter(province_name %in% geospatial_provinces)
```
...
#   left_join(farms_long %>% filter(year_type == "2019 Total"), by = "province_name")
# 
# map_total_2019 <- map_total_2019 %>%
#   filter(!is.na(count.x))
# 
# vietnam_provinces <- vietnam_provinces %>%
#   filter(province_name %in% map_total_2019$province_name)
# 
# # Create a new neighbors and weights list based on the subsetted spatial data
# neighbors<- poly2nb(vietnam_provinces, queen = TRUE)
...
-   **Moran I statistic**: `0.1239`, which is a positive value, suggests a slight tendency for spatial clustering. However, since it’s not close to 1, the clustering is weak.

#### 1. Moran I Statistic

-   The **Moran I statistic** value of `0.1239` indicates a mild positive spatial autocorrelation. This means that provinces with similar farm counts (e.g., high or low) are somewhat clustered together, though the effect is weak.
...
-   The **Expectation** of Moran’s I under the null hypothesis (random spatial distribution) is `-0.0192`. This is the average Moran’s I we’d expect if there were no spatial structure at all.

-   Your observed Moran’s I (0.1239) is higher than the expectation (-0.0192), suggesting a tendency towards clustering rather than randomness. Comparing the **Expectation** to the **Moran’s I** value helps us understand if our observed spatial pattern deviates from a random distribution.
...
-   In your result, the **Variance** is `0.0085`, which helps in calculating the standard deviation and, in turn, the significance of Moran’s I.
...
-   **p-value = 0.0604**: This indicates a 6.04% chance that the observed spatial pattern could have occurred randomly.
...
-   There is weak evidence of spatial clustering in farm counts across provinces (`Moran I = 0.1239`).

-   The positive Moran’s I, compared to the expectation under randomness (-0.0192), suggests a slight clustering effect.

-   However, the p-value of 0.0604 indicates that this result is only marginally significant, meaning we can't conclusively say that clustering exists at the 5% significance level.
